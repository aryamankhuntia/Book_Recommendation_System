...
class Recommendation:
    ...
    def recommend_book(self,book_name):
        try:
            ...
            book_pivot =  pickle.load(open(self.recommendation_config.book_pivot_serialized_objects,'rb'))
            matches = np.where(book_pivot.index == book_name)[0]
            if len(matches) == 0:
                    raise AppException(f"Book '{book_name}' not found in pivot index.", sys)
            book_id = matches[0]
...
if __name__ == "__main__":
    ...
    st.text("This is a collaborative filtering based recommendation system. It uses user ratings to recommend books based on similarity.")
